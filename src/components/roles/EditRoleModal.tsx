...
const EditRoleModal: React.FC<Props> = ({ role, onClose, onSuccess }) => {
  const [name, setName] = useState(role.name);
  const [status, setStatus] = useState(role.status);

  const handleSubmit = async () => {
    if (!name.trim()) return;
    await updateRole({ id: role.id, name, status });
    onSuccess();
    onClose();
  };
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-lg font-bold text-gray-800 mb-4">Edit Role</h2>
        <input
          value={name}
