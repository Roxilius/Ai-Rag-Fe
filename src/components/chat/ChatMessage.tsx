    ...
    >
      {!isUser && (
        <img
          ...
          alt="AI"
          className="w-8 h-8 rounded-full mr-2 border shadow-sm shrink-0"
        />
      )}

      <div
        className={`
          relative px-4 py-2 rounded-2xl text-sm sm:text-base whitespace-pre-wrap break-words shadow-md transition-all
      ...
      {isUser && (
        <img
          src={image}
          alt="User"
          className="w-8 h-8 ml-2 rounded-full border shadow-sm shrink-0"
        />
      )}
    </div>
  );
